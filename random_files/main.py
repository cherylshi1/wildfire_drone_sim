...
# Sim simplification (June 2026): total drone capability capped at 4.
# Current test case is the 2:2 ratio (2 survey + 2 water); the code always
# reads team_total_drone_count so other ratios remain possible later.
TEAM_TOTAL_DRONE_COUNT_DEFAULT = read_even_drone_count_env("REAL_SIM_TOTAL_DRONES", 4)
TEAM_WATER_DRONE_COUNT_DEFAULT = min(
    TEAM_TOTAL_DRONE_COUNT_DEFAULT,
    read_nonnegative_int_env(
        "REAL_SIM_WATER_DRONES",
        max(1, TEAM_TOTAL_DRONE_COUNT_DEFAULT // 2),
    ),
)
# Study cases: 4 drones (2S:2W, main case) or 6 drones (3S:3W, gets a
# second fire at 1:15). Ratio is locked to equal S:W.
TEAM_TOTAL_DRONE_OPTIONS = (4, 6)
TEAM_RATIO_LOCKED = True
...
FIRE_MAP_PANEL_WIDTH = 0.72
FIRE_MAP_PANEL_HEIGHT = 0.88
FIRE_MAP_PANEL_RIGHT_MARGIN = 0.04
FIRE_MAP_PANEL_TOP_MARGIN = 0.05
FIRE_MAP_VIEW_LEFT = 0.04
FIRE_MAP_VIEW_BOTTOM = 0.22
FIRE_MAP_VIEW_WIDTH = 0.64
FIRE_MAP_VIEW_HEIGHT = 0.6
...
# Suppression pacing matched to initial-attack literature (2019-2025):
# agencies target >= 95% initial attack success and ~97% of fires are
# contained during initial attack, with the first 24 h being decisive
# (Beverly et al.; MDPI Fire 2019; NWFS 2024). At 1 sim-second = 15 real
# minutes, 24 h = 96 sim-s, so ground crews alone take a detected hotspot
# from found to OUT in ~96 s (8 s response + 88 s work at 0.95/s for the
# 84 work-units to contain + extinguish once a truck is on scene). A water drone on site nearly
# triples the rate (out in ~27 s).
...
FIRE_SUPPRESSION_CONTAIN_DELAY_SECONDS = 30.0
FIRE_SUPPRESSION_EXTINGUISH_DELAY_SECONDS = 54.0
...
# of hotspots and put out roughly 65% with water support.
...
def get_survey_first_sweep_xy(slot):
    ...
    survey_count = max(survey_count, 1)
    sector_width = (SPAWN_X_MAX - SPAWN_X_MIN) / survey_count
    index = max(0, min(survey_count - 1, int(slot) - 1))
...
def get_fixed_drone_spawn_xy(role, slot):
    ...
    The map is divided into max(#survey, #water) equal x-strips, matching
    the survey search sectors. Each SURVEY drone is born EXACTLY at the first
    waypoint of its S-shaped sweep pattern (#10), so the birth point and the
    search start coincide. Its paired WATER drone spawns right beside it
    (along +x), separated by more than the crash and obstacle-avoidance radii.
    """
    survey_count, water_count = _team_counts_for_spawn()
    part_count = max(survey_count, water_count, 1)
    part_width = (SPAWN_X_MAX - SPAWN_X_MIN) / part_count
    part_index = max(0, min(part_count - 1, int(slot) - 1))
    corner_x, corner_y = get_survey_first_sweep_xy(slot)
    if role == "survey":
        return corner_x, corner_y
    pair_separation_meters = (
        max(DRONE_CRASH_RADIUS_METERS, DRONE_OBSTACLE_AVOIDANCE_RADIUS_METERS)
        + DRONE_SPAWN_PAIR_SEPARATION_MARGIN_METERS
    )
    strip_x_max = SPAWN_X_MIN + ((part_index + 1) * part_width)
    spawn_x = min(corner_x + pair_separation_meters, strip_x_max - 2.0)
...
FIRE_TRUCK_MAP_MARKER_SCALE = 1.35
FIRE_TRUCK_MAP_EMOJI = "🚒"
FIRE_TRUCK_MAP_EMOJI_ACTIVE_SCALE = 0.035
FIRE_TRUCK_MAP_EMOJI_IDLE_SCALE = 0.031
FIRE_TRUCK_MAP_EMOJI_ACTIVE_BOB = 0.006
FIRE_TRUCK_MAP_EMOJI_IDLE_BOB = 0.002
FIRE_TRUCK_SUPPRESSING_COLOR = (0.72, 0.22, 1.0, 0.98)
FIRE_TRUCK_SUPPRESSING_GLOW_COLOR = (0.52, 0.12, 1.0, 0.64)
...
def spawn_fire_truck_fleet():
    ...
    for slot in range(1, FIRE_TRUCK_COUNT + 1):
        truck_root = spawn_fire_truck_marker()
...
def set_hotspot_visual(hotspot):
    ...
    if thermal_view_enabled:
        ...
        if hotspot.current_mesh_frame_index >= 0:
            hotspot.mesh_frame_nodes[hotspot.current_mesh_frame_index].show()
        if hotspot_has_fire_truck_response(hotspot):
            hotspot.fire_cluster_color = FIRE_TRUCK_SUPPRESSING_COLOR
            hotspot.base_glow.setColor(*FIRE_TRUCK_SUPPRESSING_GLOW_COLOR)
        else:
    ...
    if thermal_view_enabled:
        ...
        if state_is_active and hotspot.detected:
            hotspot.glow.show()
            hotspot.beacon_a.show()
            hotspot.beacon_b.show()
...
tree_canopy_samples = build_tree_canopy_samples(trees, app.render)
...
fire_map_crew_truck_root = None
fire_map_crew_label = None
...
def set_team_total_drone_count(new_total):
    ...
    # Default to a balanced split; operator fine-tunes the ratio with [4]/[5].
    team_water_drone_count = max(0, team_total_drone_count // 2)
    if "status_text" in globals():
        update_status_overlay()
    if fire_map_panel is not None:
        update_fire_map_popup()
...
def choose_more_survey_drones():
    if not pregame_active or TEAM_RATIO_LOCKED:
        return
...
def format_team_ratio_menu():
    return (
        ...
        " | [1][2] survey views | [4][5] water views | [M/O] manual/auto | [H] controls"
    )
...
def update_guided_drone_motion(
    ...
    drone_motion_state.visual_pitch = degrees(drone_motion_state.pitch_radians)
    drone_motion_state.visual_roll = degrees(drone_motion_state.roll_radians)
...
def get_all_drone_views():
    """Ordered camera/control targets: survey 1-3 first, then water 1-3."""
    views = [
        {
            "root": drone,
...
def handle_number_key(key_number):
    # During mission setup the number keys configure the team; in flight they
    # switch by fixed role slots: 1-3 survey, 4-6 water.
    if pregame_active:
        if key_number == 4:
...
def build_water_dispatch_status_line():
    ...
    for slot in range(1, team_water_drone_count + 1):
        ...
        if view is not None and is_view_damaged(view):
            lines.append(f"Water {slot}: OFFLINE (crashed)")
            continue
        mode = (
        ...
        if mode == CONTROL_MODE_MANUAL:
            if is_water_slot_dispatched(slot):
                lines.append(
                    f"Water {slot}: MANUAL, dispatched earlier. Press [{key}] for AUTO response."
                )
            else:
                lines.append(
                    f"Water {slot}: MANUAL, holding. Press [{key}] for AUTO + dispatch."
                )
        elif active_target is not None or is_water_slot_dispatched(slot):
            lines.append(f"Water {slot}: AUTO + DISPATCHED, responding.")
        else:
            lines.append(f"Water {slot}: AUTO, armed for its paired survey fire.")
    return "\n".join(lines)
...
def show_collision_alert(label):
    ...
    if now > collision_alert_hide_at_seconds:
        collision_alert_labels.clear()
    collision_alert_labels.append(label.upper())
    collision_alert_hide_at_seconds = now + COLLISION_ALERT_DURATION_SECONDS
    collision_alert_text.setText("DRONE COLLISION")
    collision_alert_body_text.setText(
        " + ".join(collision_alert_labels)
        + " crashed and are OFFLINE for the rest of the round.\n"
        "Manually flown drones have no collision avoidance - keep them\n"
        "clear of other drones (within 3 m, both go down)."
    )
...
def mark_drone_damaged(role, slot):
    key = (role, slot)
    ...
    label = f"{'Survey' if role == 'survey' else 'Water'} drone {slot}"
    drone_damage_events.append((motion_state.sim_time_seconds, label))
    print(
        f"[real-sim] COLLISION: {label} damaged at "
        f"t={motion_state.sim_time_seconds:.1f}s, OFFLINE for the round."
    )
    show_collision_alert(label)
    if "status_text" in globals():
        update_status_overlay()
...
def set_full_automation():
    """[Shift+O] / blue button: hands the WHOLE team to automation in one press
    and arms automatic water dispatch for every water drone."""
...
def trigger_kill_switch():
    ...
    set_hover_all(False)
    for view in get_all_drone_views():
        mark_drone_damaged(view["role"], view["slot"])
    if "status_text" in globals():
        update_status_overlay()
...
def choose_paired_survey_water_hotspot(water_slot, reference_x, reference_y):
    """Active, detected fire that this water drone's PAIRED survey drone is
    working. A water drone follows the survey that owns the fire event; other
    water drones stay idle until their paired survey owns another fire."""
    ...
    paired = get_paired_survey_slot(water_slot)
    candidates = []
    for hotspot in active_detected_hotspots():
        if survey_slot_owns_hotspot_event(paired, hotspot):
    ...
    if water_slot in dispatched_water_slots:
        return choose_water_support_hotspot_for_slot(water_slot, reference_x, reference_y)
    return None
...
def survey_slot_owns_hotspot_event(survey_slot, hotspot):
    ...
    If an event has not been claimed yet, fall back to the drone's own sector so
    undetected-fire search still stays partitioned by region.
    """
...
def get_water_follow_anchor(water_slot):
    """Loiter point for a free water drone. Cheryl (June 14): a free water drone
    should WAIT at its birth point, not trail the survey around (the trailing
    made the routes messy and crowded the survey drone). Returning None makes
    the caller fall back to the drone's home/birth point. Dispatch pairing now
    follows the survey drone that owns a detected fire event."""
    return None
...
def choose_survey_mapping_hotspot_for_slot(survey_slot, reference_x, reference_y):
    ...
    candidates = [
        hotspot
        for hotspot in unresolved_detected_hotspots()
        if hotspot_mapping_fraction(hotspot) < 1.0
        and survey_slot_may_target_hotspot(survey_slot, hotspot)
    ]
    if not candidates:
        return None
    return min(
...
def get_water_dispatch_target(water_slot):
    ...
    if is_active_detected_hotspot(target_hotspot):
...
def choose_water_dispatch_target_for_slot(water_slot):
    ...
    candidates = (
        ...
        choose_water_support_hotspot_for_slot(water_slot, reference_x, reference_y),
        choose_detected_hotspot(reference_x, reference_y),
    )
    for candidate in candidates:
        if is_active_detected_hotspot(candidate):
            return candidate
    return None
...
def update_pregame_overlay():
    ...
    setup_header = (
        "Round complete. Choose the next team, then launch again.\n\n"
        if round_restart_pending_from_setup
        else ""
    )
    pregame_text.setText(
        f"{setup_header}"
        f"Team: {team_total_drone_count} drones"
        f"  ({survey_count} survey / {team_water_drone_count} water)"
        "    [G] toggle 4 / 6 drones\n\n"
        "HOW TO PLAY\n"
        "[W/A/S/D]  move drone      [Q] up    [E] down\n"
        "[1] [2]  survey drone views    [4] [5]  water drone views\n"
        "Click a drone on the fire map to switch to it\n"
        "[M] manual control    [O] automation    [Shift+O] all drones auto\n"
        "[V] switch first person / third person camera\n"
        "[TAB] satellite fire map (shows only detected fire)\n"
        "[P] call water 1    [L] call water 2    [K] call water 3\n"
        "[J] spray water on/off (when flying a water drone manually)\n"
        "[T] thermal view    [Z/X/C] speed    [H] show/hide controls\n"
        "Screen buttons: blue FULL AUTO, yellow HOVER ALL, red KILL\n\n"
        "Fires start hidden. Fly survey drones to find them.\n"
        "In automation each water drone trails its own survey drone\n"
        "and is sent to the fires that survey detects.\n"
        "Extra survey drones wait at spawn until you pick them\n"
        "([2], [3] or map click) and press [O].\n\n"
...
def build_pregame_overlay():
    global pregame_root, pregame_text

    pregame_root = app.aspect2d.attachNewNode("pregame_root")
    pregame_root.setBin("fixed", 200)

    # Bigger box so the full HOW TO PLAY text fits without being clipped.
    backdrop = CardMaker("pregame_backdrop")
    backdrop.setFrame(-0.9, 0.9, -0.92, 0.78)
    backdrop_np = pregame_root.attachNewNode(backdrop.generate())
    backdrop_np.setColor(0.03, 0.05, 0.08, 0.93)
    backdrop_np.setTransparency(TransparencyAttrib.MAlpha)
    backdrop_np.setDepthWrite(False)
    backdrop_np.setDepthTest(False)
    backdrop_np.setBin("fixed", 200)

    accent = CardMaker("pregame_accent")
    accent.setFrame(-0.9, 0.9, 0.72, 0.78)
    accent_np = pregame_root.attachNewNode(accent.generate())
    accent_np.setColor(1.0, 0.5, 0.18, 0.95)
    accent_np.setTransparency(TransparencyAttrib.MAlpha)
    accent_np.setDepthWrite(False)
    accent_np.setDepthTest(False)
    accent_np.setBin("fixed", 201)
    ...
    title.setBin("fixed", 202)
    title.setDepthWrite(False)
    title.setDepthTest(False)
...
CONTROLS_HELP_TEXT = (
    "CONTROLS   ([H] hide)\n"
    "[W/A/S/D] move   [Q] up   [E] down\n"
    "[1][2] survey views   [4][5] water views\n"
    "Click drone on fire map = switch view\n"
    "[M] manual   [O] auto   [Shift+O] all auto\n"
    "[V] 1st / 3rd person camera\n"
    "[TAB] satellite fire map (detected fire)\n"
    "[P][L][K] call water 1/2/3\n"
    "[J] spray water on/off (manual water drone)\n"
    "[T] thermal   [Z/X/C] speed\n"
    "Buttons: blue auto / yellow hover / red kill"
)
...
def toggle_manual_water_spray():
    ...
    if slot in manual_water_spray_slots:
        ...
        clear_water_suppression_visual(get_water_view_stream_state(view))
    else:
        if get_drone_view_control_mode(view) != CONTROL_MODE_MANUAL:
            set_drone_view_control_mode(view, CONTROL_MODE_MANUAL)
...
def build_controls_help_overlay():
    ...
    backdrop.setFrame(-0.60, -0.02, 0.02, 0.34)
    backdrop_np = controls_help_root.attachNewNode(backdrop.generate())
    backdrop_np.setColor(0.02, 0.05, 0.08, 0.78)
    backdrop_np.setTransparency(TransparencyAttrib.MAlpha)
    backdrop_np.setDepthWrite(False)
    backdrop_np.setDepthTest(False)
...
app.accept("shift-o", set_full_automation)
...
def build_collision_alert_overlay():
    global collision_alert_root, collision_alert_text, collision_alert_body_text
    ...
    backdrop.setFrame(-0.92, 0.92, -0.27, 0.24)
    backdrop_np = collision_alert_root.attachNewNode(backdrop.generate())
    backdrop_np.setColor(0.62, 0.05, 0.04, 0.9)
    backdrop_np.setTransparency(TransparencyAttrib.MAlpha)
    backdrop_np.setDepthWrite(False)
    backdrop_np.setDepthTest(False)
    backdrop_np.setBin("fixed", 152)
...
def show_results_page():
    ...
    if drone_damage_events:
        left_lines.append("")
        left_lines.append(f"COLLISIONS: {len(drone_damage_events)} drone(s) lost")
        for event_time, event_label in drone_damage_events:
            left_lines.append(
                f"  {event_label} offline at {format_round_clock(event_time)}"
            )
...
def get_water_current_target_for_view(view):
    ...
    if mode == CONTROL_MODE_MANUAL and not is_water_spray_active(view["slot"]):
        return None
    if view.get("follower") is not None:
...
def build_fire_map_crew_widget():
    """One fire-truck icon under the wind widget, with the crew
    suppression rate that is applied to every detected fire (#8, Cheryl June 14)."""
    global fire_map_crew_truck_root, fire_map_crew_label
    crew_x = FIRE_MAP_WIND_WIDGET_X
    crew_z = FIRE_MAP_WIND_WIDGET_Z - 0.058   # right under the wind widget
    fire_map_crew_truck_root = fire_map_panel.attachNewNode("fire_map_crew_truck_root")
    fire_map_crew_truck_root.setPos(crew_x, 0.0, crew_z)
    fire_map_crew_truck_root.setScale(1.7)    # bigger truck icon
    fire_map_crew_truck_root.setDepthWrite(False)
    fire_map_crew_truck_root.setDepthTest(False)
    fire_map_crew_truck_root.setBin("fixed", 121)
    truck = build_fire_map_truck_marker(fire_map_crew_truck_root)
    truck.setPos(0.0, 0.0, 0.0)
    truck.show()
    fire_map_crew_label = OnscreenText(
        text="",
        parent=fire_map_panel,
        pos=(crew_x + 0.066, crew_z + 0.006),
        scale=0.024,
        fg=(0.96, 0.86, 0.82, 1.0),
        align=TextNode.ALeft,
        mayChange=True,
    )
    fire_map_crew_label.setBin("fixed", 121)
    fire_map_crew_label.setDepthWrite(False)
    fire_map_crew_label.setDepthTest(False)


def build_fire_map_legend():
    legend_y = FIRE_MAP_VIEW_BOTTOM - 0.031
    legend_items = (
        ((1.0, 0.55, 0.3, 1.0), "fire"),
        ((1.0, 0.82, 0.4, 1.0), "need W"),
        ((0.5, 0.85, 1.0, 1.0), "water"),
        (FIRE_TRUCK_SUPPRESSING_COLOR, "truck"),
        ((0.46, 0.96, 0.56, 1.0), "out"),
    )
    box_half = 0.012
    x = FIRE_MAP_VIEW_LEFT
    ...
    mode_text = OnscreenText(
        ...
        parent=fire_map_panel,
        pos=(FIRE_MAP_VIEW_LEFT, legend_y - 0.05),
        scale=0.024,
    ...
    tab_text = OnscreenText(
        text="Press [TAB] to open/close map",
        parent=fire_map_panel,
        pos=(FIRE_MAP_VIEW_LEFT, legend_y - 0.086),
        scale=0.027,
...
def build_fire_map_overlay():
    ...
    fire_map_panel.setPos(
        -FIRE_MAP_PANEL_WIDTH - FIRE_MAP_PANEL_RIGHT_MARGIN,
        0.0,
        -(FIRE_MAP_PANEL_TOP_MARGIN + FIRE_MAP_PANEL_HEIGHT),
    )
...
def update_manual_water_drone(dt, current_speed_units_per_second, input_active=True):
    ...
    if input_active:
        desired_vx, desired_vy, desired_vz = compute_desired_velocity(
            camera_angle,
            key_map,
    ...
    if closest_hotspot is not None and is_water_spray_active(1):
        ...
        register_hotspot_water_drone_contact(closest_hotspot, is_engaged)

    update_water_suppression_visual(team_state.water_target_hotspot)
...
def update_water_drone(dt):
    ...
    register_hotspot_water_drone_contact(target_hotspot, False)
    water_drone_state.mission_active = True
    hotspot_id = id(target_hotspot)
    ...
    if (
        ...
        target_hotspot.water_drone_engaged = True
        target_hotspot.water_drone_engagement_count = max(
            target_hotspot.water_drone_engagement_count,
            1,
        )

    update_water_suppression_visual(target_hotspot)


def get_unprotected_drone_roots():
    """Roots of drones that get NO separation assist: manually flown drones
    (collision avoidance off in manual) and already-offline drones."""
    unprotected = set()
    for view in get_all_drone_views():
        root = view.get("root")
        if root is None or root.isEmpty():
            continue
        if (
            get_drone_view_control_mode(view) == CONTROL_MODE_MANUAL
            or is_view_damaged(view)
        ):
...
def check_manual_collisions():
    ...
    automated, undamaged drones are excluded (their assist keeps them apart)."""
    views = [
        view
        for view in get_all_drone_views()
        if view.get("root") is not None and not view["root"].isEmpty()
    ]
    for first_index in range(len(views)):
        first = views[first_index]
        first_unassisted = (
            get_drone_view_control_mode(first) == CONTROL_MODE_MANUAL
            or is_view_damaged(first)
        )
        for second in views[first_index + 1:]:
            second_unassisted = (
                get_drone_view_control_mode(second) == CONTROL_MODE_MANUAL
                or is_view_damaged(second)
            )
            if not (first_unassisted or second_unassisted):
                continue
            if is_view_damaged(first) and is_view_damaged(second):
                continue
            ...
            if (
                ...
                mark_drone_damaged(first["role"], first["slot"])
                mark_drone_damaged(second["role"], second["slot"])
...
def update_fire_map_hotspot_nodes(visible_hotspots):
    ...
    for hotspot_index, hotspot in enumerate(visible_hotspots, start=1):
        ...
        if hotspot.suppression_state == FIRE_STATE_ACTIVE:
            ...
            if hotspot_has_fire_truck_response(hotspot):
                label_text.setText("")
                flame_node.setColorScale(0.72, 0.28, 1.0, 1.0)
                halo_node.setColorScale(0.55, 0.16, 1.0, 0.55)
                cross_node.setColorScale(0.86, 0.48, 1.0, 1.0)
                truck_node.show()
            elif hotspot.water_drone_engaged:
                label_text.setText("")
                flame_node.setColorScale(0.5, 0.85, 1.0, 1.0)
                halo_node.setColorScale(0.4, 0.8, 1.0, 0.5)
                cross_node.setColorScale(0.46, 0.92, 1.0, 1.0)
                truck_node.hide()
            elif hotspot.water_drone_requested:
                label_text.setText("")
                flame_node.setColorScale(1.0, 0.82, 0.4, 1.0)
                halo_node.setColorScale(1.0, 0.7, 0.25, 0.5)
                cross_node.setColorScale(1.0, 0.86, 0.28, 1.0)
                truck_node.hide()
            else:
                label_text.setText("")
                flame_node.setColorScale(1.0, 0.55, 0.3, 1.0)
                halo_node.setColorScale(1.0, 0.4, 0.18, 0.5)
                cross_node.setColorScale(1.0, 0.36, 0.16, 1.0)
                truck_node.hide()
            ...
            label_text.setText("")
            if hotspot_has_fire_truck_response(hotspot):
                flame_node.setColorScale(0.72, 0.28, 1.0, 0.9)
                cross_node.setColorScale(0.86, 0.48, 1.0, 1.0)
                truck_node.show()
            else:
                flame_node.setColorScale(1.0, 0.82, 0.4, 0.85)
                cross_node.setColorScale(1.0, 0.84, 0.42, 0.98)
                truck_node.hide()
            ...
            label_text.setText("")
            truck_node.hide()
            cross_node.setColorScale(0.46, 0.96, 0.56, 0.98)
            ...
            label_text.setText("")
            truck_node.hide()
            cross_node.setColorScale(0.58, 0.18, 0.14, 0.9)
            cross_node.setScale(0.8)
...
def build_fire_map_truck_node(truck_state):
    ...
    marker_node.setScale(FIRE_TRUCK_MAP_MARKER_SCALE)
    marker_node.setBin("fixed", 119)
    label_text = OnscreenText(
        text="",
        parent=fire_map_panel,
        pos=(0.0, 0.0),
        scale=FIRE_TRUCK_MAP_EMOJI_IDLE_SCALE,
        fg=(1.0, 0.86, 0.86, 0.96),
        align=TextNode.ACenter,
        mayChange=True,
    )
    label_text.setBin("fixed", 124)
...
def update_fire_map_truck_nodes():
    ...
    for truck_state in fire_truck_states:
        ...
        if truck_state.target_hotspot is not None:
            marker_node.setColorScale(0.9, 0.46, 1.0, 1.0)
            label_text.setFg((0.96, 0.72, 1.0, 0.98))
            emoji_scale = FIRE_TRUCK_MAP_EMOJI_ACTIVE_SCALE
            bob_amount = FIRE_TRUCK_MAP_EMOJI_ACTIVE_BOB
            bob_speed = 6.0
        else:
            marker_node.setColorScale(1.0, 1.0, 1.0, 1.0)
            label_text.setFg((1.0, 0.86, 0.86, 0.96))
            emoji_scale = FIRE_TRUCK_MAP_EMOJI_IDLE_SCALE
            bob_amount = FIRE_TRUCK_MAP_EMOJI_IDLE_BOB
            bob_speed = 2.4
        emoji_phase = motion_state.sim_time_seconds * bob_speed + (truck_state.slot * 1.3)
        label_text.setText(FIRE_TRUCK_MAP_EMOJI)
        label_text.setScale(emoji_scale)
        label_text.setPos(
            clamp(
                map_x + 0.012 + (sin(emoji_phase) * bob_amount),
                FIRE_MAP_VIEW_LEFT + 0.018,
                FIRE_MAP_VIEW_LEFT + FIRE_MAP_VIEW_WIDTH - 0.018,
            ),
            clamp(
                map_y - 0.004 + (abs(cos(emoji_phase)) * bob_amount),
...
def update_fire_map_popup():
    ...
    fire_map_status_text.setText(
        "Detected markers only. Undetected fire stays hidden."
    )
    # Fire-truck status under the wind widget: limited trucks,
    # not one crew magically assigned to every detected fire.
    if fire_map_crew_label is not None:
        assigned_count = sum(
            1
            for truck_state in fire_truck_states
            if truck_state.target_hotspot is not None
        ...
        engaged_count = sum(
            1
            for truck_state in fire_truck_states
            if truck_state.target_hotspot is not None and truck_state.engaged
        )
        fire_map_crew_label.setText(
            "fire trucks %d/%d\nworking %d"
            % (assigned_count, len(fire_truck_states), engaged_count)
        )
...
def format_selected_drone_status():
    ...
    if view is None:
        return "VIEW: (none)"
    if is_view_damaged(view):
        return f"VIEW: {view['label']}  [OFFLINE - collision damage]"
    mode = get_drone_view_control_mode(view)
    ...
    return f"VIEW: {view['label']}  [{mode_label}]{spray_label}"
...
def run_water_dispatch_self_test():
    ...
    assert water_dispatch_targets.get(1) is hotspot
    assert get_drone_view_control_mode(water_2_view) == CONTROL_MODE_MANUAL
    reset_hotspot_water_drone_contact()
